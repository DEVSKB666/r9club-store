  ...
  try {
    ...
    for (const legacyUser of legacyUsers) {
      try {
        ...
        if (existingUser) {
          results.skipped++;
          results.details.push(`⏭️ Skipped: ${legacyUser.email} (exists)`);
          continue;
        }
        ...
        results.imported++;
        results.details.push(`✅ Imported: ${legacyUser.username} (${legacyUser.email}) - ฿${legacyUser.credit}`);
      } catch (error) {
        results.errors++;
        results.details.push(`❌ Error: ${legacyUser.email} - ${error}`);
      }
    }

    return Response.json({
      success: true,
      message: `Migration completed: ${results.imported} imported, ${results.skipped} skipped, ${results.errors} errors`,
